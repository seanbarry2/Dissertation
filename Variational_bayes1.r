#  Sean Barry's own version of the supplementary material script from Gallinat and Pearse 2021

# For parallelizing code:
# Detects how many cores the device has
num_cores <- parallel::detectCores()

# Set the number of cores for Stan
...
one.three.stan.model <- vb(object=sm_one_three,
                             data=list(Ntotal=nrow(cons.env), Nspp=n.spp,
                                       presence=cons.env$presence,
                                       env=cons.env$env, spp=cons.env$num_species, Vphy=vcv(tree)),
                             algorithm = "meanfield", tol_rel_obj = 0.0001, iter = 50000, seed=123456)
...
two.one.stan.model <- vb(object=sm_two_one,
                           ...
                           algorithm = "meanfield", seed=123456)
...
two.two.stan.model <- vb(object=sm_two_two,
                           ...
                           algorithm = "fullrank", tol_rel_obj = 0.00001, iter = 20000, seed=123456)
...
real<lower=0, upper=10> lam_int;
real<lower=0, upper=10> lam_slp;
real<lower=0> null_int;
real<lower=0> null_slp;
real mean_int;
real mean_slp;
// Coefficients for the partner-derived variance in model terms
real<lower=0, upper=10> lam_partner_int;     // (phylogenetic component)
...
# looks like
three.one.stan.model.base <- vb(object=sm_three_one,
                                  ...
                                  algorithm = "meanfield", tol_rel_obj = 0.001, iter = 50000, seed=123456)
...
three.one.stan.model.partner <- vb(object=sm_three_one,
